...
* **Firebase:** Use the `Result<T, E>` pattern for initialization. Do not export raw `auth` or `db` objects that might be undefined or throw during module evaluation.
* **Evaluation Safety:** Ensure all environment-dependent modules (like Firebase or external API clients) are initialization-safe. Prefer a `firebaseResult` export that components can check before attempting to use services.
